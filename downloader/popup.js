...
function OnUltimate( ev)
{
    ...
    xmlHttpReq.open("GET", "http://keepvid.com/?url=https://www.youtube.com/watch?v=Jgk3u44W2i4", true);
...
{  		
 ...
 //        console.log(j);
 //        if (j >= 0) {
 //            var curTabId = parseInt(query.slice(j + 7));
 //            chrome.extension.sendMessage({msg:"getYoutubeUrl", tabId: curTabId}, function(response) {
 //                if (response.url) {
 ...
 //                        if (xhr.readyState === 4) {
 //                            addYoutubeVideoLink(xhr.responseText);    
 //                        }
 //                    }
 //                }
 //            });
...
textEN['idvideo'] = "Please wait for the link....";
...
function addYoutubeVideoLink(url) {
    var el = document.createElement('html');
    el.innerHTML  = url;
    var links = el.getElementsByClassName('l')
    //console.log(url);
    if (links) {
        var title = {};
        title.url = links[0].href;
        title.title = links[0].download;
        title.mime = "mp4";
        if (!videoUrls) {
            videoUrls = [];
        }
        videoUrls.push(title);
        showVideoUrls();
    }
}